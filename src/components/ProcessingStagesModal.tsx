    ...
    const interval = setInterval(() => {
      setCurrentStage(prev => {
        if (prev >= stages.length - 1) {
          ...
          setTimeout(() => {
            onComplete();
          }, 1000); // Wait a second after the last stage before completing
          return prev;
        }
        return prev + 1;
      });
              ...
              currentStage === index && (
                ...
                >
                  <motion.div
                    ...
                    animate={{ scale: 1 }}
                    transition={{ duration: 0.5 }}
                    className="p-4 rounded-full bg-brand-myrtleGreen/10"
                  >
                    <stage.icon className="w-12 h-12 text-brand-myrtleGreen" />
                  </motion.div>
                  <h3 className="text-xl font-serif text-brand-myrtleGreen">
                    {stage.title}
                  </h3>
                  <p className="text-brand-jet/70">
                    {stage.description}
                  </p>
                </motion.div>
              )
